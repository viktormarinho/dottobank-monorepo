...
const PixRoute = () => {
    ...
    const handlePix = async () => {
        ...
        axios.post('http://localhost:4000/app/pix', { to: toId, value: depositValue }, { headers: getAuthHeaders() })
            .then(res => {
                Swal.mixin({
                    toast: true,
                    position: 'bottom',
    ...
    return (
        <div className="w-full flex flex-col items-center gap-8 mt-16">
            ...
            <h1 className="text-center text-2xl px-4">Digite o valor do pix a ser enviado.</h1>
            ...
            <div className="w-full md:max-w-sm flex flex-col gap-4 px-16">
                <button onClick={handlePix} className="text-white text-xl bg-[#3F83A9] hover:bg-[#377294] rounded-lg py-1 w-full">Enviar pix</button>
